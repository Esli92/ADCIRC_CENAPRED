...
start_date = obse.index[0]
end_date = obse.index[-1]
...
elev_series = obse.reindex(dates_full,fill_value='NaN',method=None)
...
#Now "resample" the time series so it has the same lenght as the previously made date range list. 
fore_series = fore.reindex(elev_series.index,fill_value='NaN',method=None)
